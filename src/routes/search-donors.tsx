...
import { KAKINADA_AREAS, BLOOD_GROUPS, type Donor } from "@/lib/blood-data";
...
function SearchDonors() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      let q = supabase.from("donors").select("*").order("name");
      if (bg) q = q.eq("blood_group", bg);
      if (area) q = q.eq("area", area);
      if (onlyAvail) q = q.eq("available", true);
      const { data } = await q;
          ...
          <div className="mt-8 bg-card p-4 md:p-6 rounded-xl border border-border shadow-sm grid md:grid-cols-4 gap-3">
            ...
            <div className="flex items-end">
              <div className="w-full text-center bg-primary/5 border border-primary/20 text-primary rounded-md py-2 text-sm font-semibold flex items-center justify-center gap-2">
                <Search className="w-4 h-4" /> {loading ? "…" : donors.length} donors
              </div>
            </div>
          </div>
        </div>
          ...
          {donors.map((d) => (
            <div key={d.id} className="bg-card border border-border rounded-xl p-5 hover:shadow-lg hover:border-primary transition">
              <div className="flex items-start justify-between">
                ...
                <div className="w-12 h-12 rounded-lg bg-primary text-primary-foreground grid place-items-center font-bold">{d.blood_group}</div>
              </div>
